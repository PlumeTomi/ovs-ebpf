...
__inline__ static int ovs_parser(struct __sk_buff* skb, unsigned int *ifc) {
    ...
    *ifc = skb->ifindex;
    ...
    if (((unsigned char) hdrs.ethernet.srcAddr[0]) == 0x6a &&
        ((unsigned char) hdrs.ethernet.srcAddr[1]) == 0x08 &&
        ((unsigned char) hdrs.ethernet.srcAddr[2]) == 0x60 &&
        ((unsigned char) hdrs.ethernet.srcAddr[3]) == 0x45 &&
        ((unsigned char) hdrs.ethernet.srcAddr[4]) == 0x25 &&
        ((unsigned char) hdrs.ethernet.srcAddr[5]) == 0x41) {
        // packet came from outside GRE
        return 0;
    } else if (
        ((unsigned char) hdrs.ethernet.srcAddr[0]) == 0x8a &&
        ((unsigned char) hdrs.ethernet.srcAddr[1]) == 0xd2 &&
        ((unsigned char) hdrs.ethernet.srcAddr[2]) == 0x58 &&
        ((unsigned char) hdrs.ethernet.srcAddr[3]) == 0xcb &&
        ((unsigned char) hdrs.ethernet.srcAddr[4]) == 0xa7 &&
        ((unsigned char) hdrs.ethernet.srcAddr[5]) == 0xb6) {
        // packet came from inside GRE
        return 1;
    } else {
        // packet not from GRE, drop it
        return -1;
    }
